...
def plot_loghist(x, nbins,cutoff=8.):
  ...
  bins = [ pow(dexp,k) for k in range(0,2*N) ]
...
nbins =  20
yy,xx = plot_loghist(d_in,nbins)
for d,U in zip(xx,yy): print("d: ",d," -> ",U)

xxappr = np.linspace(10.,320.)
yyappr = 10**4 * np.power(xxappr,-1.)

plt.plot(xxappr,yyappr)
