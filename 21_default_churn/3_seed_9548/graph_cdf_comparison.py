...
lab = lambda i: "X" if i==0 else "Y"

def plot_curve_data(data,fig=None,ax=None, j=0, force_N : int = 0, first_row : bool = False):
    N = data.shape[1]
    V = data.shape[0]
    if (force_N!=0):
        V = force_N
    ...
    for i in range(N):
        ...
        xx = sorted(data[:,i])
        ax.plot(xx,y_axis,color=color(),label=lab(j)) ; curves_plotted+=1
...
for i,f in enumerate(files):
    ...
    if (i==0):
        N = get_number_rows(d)
    fig,ax = plot_curve_data(d,fig,ax,i,V)
